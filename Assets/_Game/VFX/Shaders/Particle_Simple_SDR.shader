...
Shader "Spellcards/VFX/Particle/Simple"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv_MainTex10 = IN.ase_texcoord2.xy;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float2 uv_MainTex10 = IN.ase_texcoord.xy;
				

				surfaceDescription.Alpha = ( IN.ase_color.a * saturate( ( tex2D( _MainTex, uv_MainTex10 ) * _OpacityIntensity ) ) ).r;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float2 uv_MainTex10 = IN.ase_texcoord1.xy;
